...
class MotionAwareWanTrainingModule(DiffusionTrainingModule):
    ...
    def forward(self, data, inputs=None, return_loss_dict=False):
        ...
        if total_loss is None:
            ...
            # Return a dummy zero loss so training can continue with other samples
            print(f"[WARNING] Skipping sample - no target data for training mode '{self.training_mode}'")

            # Create a dummy loss with gradient connection to trainable params
            dummy_param = None
            if self.enable_depth and hasattr(self, 'depth_head'):
                for p in self.depth_head.parameters():
                    if p.requires_grad:
                        dummy_param = p
                        break
            if self.enable_motion and hasattr(self, 'motion_head') and dummy_param is None:
            ...
            if dummy_param is not None:
                # Create zero loss connected to params so backward() works
                total_loss = 0.0 * dummy_param.sum()
            else:
                total_loss = torch.tensor(0.0, device=self.pipe.device, dtype=self.pipe.torch_dtype, requires_grad=True)
    ...
    for epoch_id in range(num_epochs):
        ...
        for data in progress_bar:
            with accelerator.accumulate(model):
                ...
                # Check if sample was skipped (missing target data)
                sample_skipped = loss_dict.get("skipped", False)

                # Collect losses for averaging (skip if sample was skipped)
                ...
                if use_wandb and accelerator.is_main_process and global_step % log_every_n_steps == 0 and not sample_skipped:
                    ...
                    if global_step <= log_every_n_steps * 3:
                        print(f"[Step {global_step}] Logged to wandb: total={loss_dict['total_loss']:.4f}, "
                              f"noise={loss_dict.get('noise_loss', 0):.4f}, "
