...
class WAN22MotionDepthTrainingModule(DiffusionTrainingModule):
    ...
    def forward_preprocess(self, data):
        ...
        # Extra inputs (input_image/depth_image for TI2V)
        for extra_input in self.extra_inputs:
            if extra_input == "input_image":
                inputs_shared["input_image"] = video_frames[0]
            elif extra_input == "end_image":
                inputs_shared["end_image"] = video_frames[-1]
            elif extra_input == "depth_image":
                if "depth_image" in data:
                    inputs_shared["depth_image"] = data["depth_image"]
                elif depth_maps is not None:
                    inputs_shared["depth_image"] = self._depth_tensor_to_pil(depth_maps[0, 0])
            elif extra_input == "depth_video":
                if "depth_video" in data:
                    inputs_shared["depth_video"] = data["depth_video"]
                elif depth_maps is not None:
                    inputs_shared["depth_video"] = [
                        self._depth_tensor_to_pil(depth_maps[0, i]) for i in range(depth_maps.shape[1])
                    ]
    ...
    def _depth_tensor_to_pil(self, depth_frame: torch.Tensor) -> Image.Image:
        depth = depth_frame.detach().float()
        depth_min = depth.min()
        depth_max = depth.max()
