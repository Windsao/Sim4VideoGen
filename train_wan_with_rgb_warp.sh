...
# Training script for WAN model with RGB warp loss (photometric temporal consistency)
#
# This script trains the WAN model with RGB warp loss in addition to the standard
# motion and depth losses. RGB warp loss enforces temporal consistency by warping
# RGB frames using optical flow and comparing with target frames.
#
# RGB warp loss is useful for:
# - Improving temporal consistency in video generation
# - Learning better motion-aware representations
# - Regularizing the model to respect optical flow constraints
...
# Usage: ./train_wan_with_rgb_warp.sh [source_dir] [training_mode]
#    or: ./train_wan_with_rgb_warp.sh [training_mode]  (uses default source_dir)
#
# Training modes:
...
# Warp loss parameters (depth temporal consistency via flow warping)
USE_WARP_LOSS=true
...
RGB_WARP_LOSS_TYPE="l1"  # "mse", "l1", "smooth_l1", "charbonnier"
RGB_WARP_USE_SSIM=false   # Set to true to combine with SSIM loss
RGB_WARP_SSIM_WEIGHT=0.85  # Weight for SSIM when enabled

# Update output path based on loss settings
if [ "${USE_WARP_LOSS}" = true ]; then
    OUTPUT_MODEL_PATH="${OUTPUT_MODEL_PATH}_warp_loss"
fi
if [ "${USE_RGB_WARP_LOSS}" = true ]; then
    OUTPUT_MODEL_PATH="${OUTPUT_MODEL_PATH}_rgb_warp"
fi
...
echo "========================================="
echo "WAN Training with RGB Warp Loss"
echo "========================================="
echo ""
...
echo ""
echo "Warp loss settings (depth temporal consistency):"
...
echo ""
echo "RGB Warp loss settings (photometric temporal consistency):"
echo "  - RGB warp loss enabled: ${USE_RGB_WARP_LOSS}"
if [ "${USE_RGB_WARP_LOSS}" = true ]; then
    echo "  - RGB warp loss weight: ${RGB_WARP_LOSS_WEIGHT}"
    echo "  - RGB warp loss type: ${RGB_WARP_LOSS_TYPE}"
    echo "  - Use SSIM: ${RGB_WARP_USE_SSIM}"
    if [ "${RGB_WARP_USE_SSIM}" = true ]; then
        echo "  - SSIM weight: ${RGB_WARP_SSIM_WEIGHT}"
    fi
fi
echo ""
...
# Update wandb run name based on loss settings
if [ "${USE_WARP_LOSS}" = true ]; then
    wandb_run_name="${wandb_run_name}_warp_loss"
fi
if [ "${USE_RGB_WARP_LOSS}" = true ]; then
    wandb_run_name="${wandb_run_name}_rgb_warp"
fi
if [ "${USE_SPATIOTEMPORAL_DEPTH}" = true ]; then
...
# ============================================
# Step 2: Run training with all losses
# ============================================
echo "Step 2/2: Starting training with motion, depth, warp, and RGB warp losses..."
echo ""
...
# Build RGB warp loss arguments
RGB_WARP_ARGS=""
if [ "${USE_RGB_WARP_LOSS}" = true ]; then
    RGB_WARP_ARGS="--use_rgb_warp_loss --rgb_warp_loss_weight ${RGB_WARP_LOSS_WEIGHT} --rgb_warp_loss_type ${RGB_WARP_LOSS_TYPE}"
    if [ "${RGB_WARP_USE_SSIM}" = true ]; then
        RGB_WARP_ARGS="${RGB_WARP_ARGS} --rgb_warp_use_ssim --rgb_warp_ssim_weight ${RGB_WARP_SSIM_WEIGHT}"
    fi
fi
