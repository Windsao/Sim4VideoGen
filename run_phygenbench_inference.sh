...
# PhyGenBench Benchmark Inference Script for Wan2.1-T2V-1.3B
# This script runs the Wan2.1-T2V-1.3B model on PhyGenBench test cases
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}PhyGenBench Benchmark Wan2.1-T2V-1.3B Pipeline${NC}"
echo -e "${BLUE}================================================${NC}"

# Configuration
OUTPUT_DIR="phygenbench_results"
MODEL_NAME="wan21_t2v_1.3b"
MAX_SAMPLES=""  # Leave empty for all samples, or set a number like "10"
NUM_FRAMES=81   # 81 frames at 16fps = ~5 seconds
FPS=16
HEIGHT=480
WIDTH=832
CFG_SCALE=7.0
...
SHARD_INDEX=""
NUM_SHARDS=""
PROMPTS_JSON="phygenbench_prompts.json"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --output-dir DIR       Output directory (default: phygenbench_results)"
            echo "  --model-name NAME      Model name for output folder (default: wan21_t2v_1.3b)"
            echo "  --prompts-json FILE    Prompts JSON file (default: phygenbench_prompts.json)"
...
SCRIPT_PATH="inference_phygenbench_wan22.py"
...
if [ ! -f "$PROMPTS_JSON" ]; then
    echo -e "${RED}Error: $PROMPTS_JSON not found!${NC}"
    echo "Please ensure the prompts JSON file is in the current directory."
    exit 1
fi
...
CMD="$CMD --prompts_json $PROMPTS_JSON"
...
{
    echo "================================================================"
    echo "PhyGenBench WAN2.1-T2V-1.3B Inference Log"
...
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    ...
    echo "  Inference report: $OUTPUT_DIR/phygenbench_inference_report.md"
    echo "  Log file: $LOG_FILE"
    echo ""
    echo -e "${YELLOW}Next Steps:${NC}"
    echo "  1. Review generated videos in: $OUTPUT_DIR/.$MODEL_NAME/"
    echo "  2. Check the report for any failed generations"
...
if [ -f "$OUTPUT_DIR/phygenbench_inference_report.md" ]; then
    echo ""
    echo -e "${BLUE}Report Summary:${NC}"
    tail -n 20 "$OUTPUT_DIR/phygenbench_inference_report.md"
fi
